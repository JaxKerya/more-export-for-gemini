...
* **Çok Dilli ve Sağdan-Sola (RTL) Desteği**: Çıkarıcı, raporun dilini (`document.documentElement.lang`) ve baskın yazı yönünü (`GEP.extractor.detectDir`; Arapça, İbranice, Farsça, Urduca, Süryanice, Thaana, NKo ve Arapça/İbranice sunum biçimlerini kapsar) tespit edip IR'a ekler. Bu `lang`/`dir` tüm gösterim formatlarına akar: HTML/Reader/PDF kök `<html lang dir>` ayarlar ve her bloğu (`<p>`, başlıklar, `<li>`, `<th>/<td>`, `<figcaption>`) `dir="auto"` ile işaretler; ilgili CSS, kenar/dolgu/hizalamada mantıksal özelliklere (`padding-inline-start`, `border-inline-start`, `text-align:start`) geçirilerek RTL'de doğru yöne döner. EPUB `<dc:language>` ile XHTML kökünde `xml:lang`/`lang`/`dir` ve `<body dir>` üretir; DOCX, `styles.xml` varsayılanlarını `<w:bidi/>`/`<w:rtl/>` + sağa hizalama yapar ve tablolara `<w:bidiVisual/>` ekler. Latin dışı (CJK, Kiril, Yunan…) içerik zaten yakalanıyordu; bu özellik onların **doğru gösterimini** sağlar. `lang`/`dir` taşımayan IR'lar için geriye dönük uyumludur (lang'sız, `ltr`). **LaTeX** de artık kapsam içinde: preamble, RTL yazım tespit edildiğinde bidi algoritmasını (`babel` + `bidi=basic`), yazımı kapsayan bir fontu (Arapça için Amiri, İbranice için David CLM; `\IfFontExistsTF` ile korumalı, Noto yedekli, üç font ailesine de bağlı) ve Arapça harf bitişikliğini birlikte kurar. `onchar=ids fonts` karakter bazında dil/font değiştirdiği için Türkçe bir rapordaki Arapça alıntı gövdede hiçbir işaretleme gerektirmez; belge yönü ise yalnızca RTL metin raporda baskınsa çevrilir. XeLaTeX için `polyglossia` kurulumu RTL-baskın raporları kapsar, LuaLaTeX önerilir. **Bilinen sınır:** LaTeX'te Kiril ve Çince metin, kullanılan TeX dağıtımında uygun font yoksa hâlâ eksik glif uyarısı verebilir; derleme başarılı olur, uyarılar `npm run validate:corpus` çıktısında görünür.
