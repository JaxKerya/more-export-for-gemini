...
* **Dışa Aktarma Formatları**:
  * **Metin Formatları**: Markdown (.md) [GFM, CommonMark, Obsidian ve Notion biçimleri], Düz Metin (.txt), HTML (.html), JSON (.json), LaTeX (.tex), reStructuredText (.rst), AsciiDoc (.adoc), Typst (.typ), Org Mode (.org).
  * **Akademik Yayıncılık Formatları**: Quarto (.qmd), Marp slayt destesi (.md).
  * **Veri ve Yapılandırılmış Formatlar**: CSV (.csv), OPML (.opml), YAML (.yaml), Jupyter Notebook (.ipynb), XML/TEI (.xml).
  * **Akademik Referans Formatları**: BibTeX (.bib), RIS (.ris), CSL-JSON (.json) [Zotero/Pandoc citeproc].
  * **Zengin Doküman Formatları**: Word (.docx) [OpenXML standartlarında], OpenDocument Text (.odt) [LibreOffice/OpenOffice/Google Docs], PDF (.pdf) [yazdırma arayüzü ile], EPUB (.epub) [e-kitap paketi olarak].
* **Matematik/Denklem Desteği**: KaTeX/MathML formülleri (LaTeX kaynağı, varsa MathML ve KaTeX'in hâlihazırda render edilmiş HTML'i) yakalanır. HTML/PDF/EPUB çıktıları, formül içerdiklerinde gömülü KaTeX stil sayfasını (woff2 fontları base64 olarak gömülü) ekleyerek denklemi tamamen çevrimdışı, ağ gerektirmeden ve kaynak sayfadakiyle birebir aynı gösterir. Yerleşik bir LaTeX dönüştürücüsü (`src/lib/texmath.js`) sayesinde DOCX'te gerçek Office Math (OMML), ODT'de gömülü MathML formül nesnesi ve Typst'te Typst matematik sözdizimi üretilir. Ayrıca Markdown/Quarto/Marp/Jupyter'da `$…$`/`$$…$$`, LaTeX/Org'da `\(…\)`/`\[…\]`, reStructuredText'te `:math:`, XML/TEI'de `<formula>` olarak işlenir.
...
* **Doküman Meta Verileri**: Ayarlardan girilen isteğe bağlı Yazar, Kurum, Anahtar Kelimeler ve Özet (Abstract) alanları; destekleyen formatlara yerel olarak işlenir — LaTeX `\author`/`abstract`/anahtar kelimeler, DOCX `docProps/core.xml`, ODT `meta.xml`, EPUB OPF (`dc:creator`/`dc:subject`/`dc:description`), HTML/PDF `<meta>` etiketleri ile yazar/özet/anahtar kelime bloğu, Quarto YAML frontmatter ve Obsidian front matter. Boş bırakılan alanlar çıktıya eklenmez.
...
* **Kod Bloğu Dil Etiketi**: Sayfadan tespit edilen programlama dili (`language-…`/`lang-…`/`data-language`) Markdown, reStructuredText, AsciiDoc, Org ve HTML çıktılarında korunur.
...
* **src/lib/**:
  ...
  * `settings.js`: Kullanıcı tercihlerini saklamak ve tarayıcı profiliyle eşitlemek için `chrome.storage.sync` entegrasyonu.
...
* **src/vendor/**: Üçüncü taraf statik varlıklar. `katex.js`, KaTeX stil sayfasını woff2 fontları base64 olarak gömülü hâlde içeren, derleme zamanı üretilen (bkz. `scripts/build-katex.mjs`) tek dosyalık bir modüldür; HTML/PDF/EPUB çıktılarına çevrimdışı matematik için inline edilir.
* **src/exporters/**: Ara temsil nesnesini hedef dosya türlerine dönüştüren, harici bağımlılığı olmayan JS dönüştürücüleri:
  ...
  * `odt.js`: `mimetype`, `META-INF/manifest.xml`, `content.xml`, `styles.xml` ve `meta.xml` parçalarını birleştirerek geçerli bir OpenDocument Text (`.odt`) paketi üreten dönüştürücü.
  * Diğer formatlara özel metin tabanlı dönüştürücüler (`markdown.js`, `latex.js`, `rst.js`, `typst.js`, `csljson.js`, `quarto.js`, `marp.js` vb.).
* **src/options/**: Eklenti ayarlar sayfasının arayüz ve mantıksal kodları.
* **src/popup/**: Tarayıcı araç çubuğunda uzantı simgesine tıklandığında açılan hızlı durum ve bilgi penceresi.
* **test/**: Node.js sanal makine (`vm`) ortamında çalışan otomatik test betikleri (`validate.mjs`, `edge-cases.mjs` ve `linkedom` ile gerçek çıkarıcıyı sınayan `extractor.mjs`) ve sentetik DOM örnekleri (`test/fixtures/`).
